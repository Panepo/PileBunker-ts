export interface LinkInterface {
  text: string;
  link: string;
}

export const listLink = [
  { text: '城プロRE', link: 'http://www.dmm.com/netgame_s/oshirore/' },
  { text: '攻略wiki', link: 'http://scre.swiki.jp/' },
  {
    text: 'くろもじ屋',
    link: 'http://www116.sakura.ne.jp/~kuromoji/castle_enemy.htm'
  },
  { text: 'ダメージ計算機', link: 'http://cascal.html.xdomain.jp/cascal/' }
];

export const listDrawer = [
  { text: '御城プロジェクト：RE', link: 'http://www.dmm.com/netgame_s/oshirore/' },
  { text: '攻略 Wiki', link: 'http://scre.swiki.jp/' },
  { text: 'くろもじ屋', link: 'http://www116.sakura.ne.jp/~kuromoji/castle_enemy.htm' },
  { text: 'React', link: 'https://facebook.github.io/react/' },
  { text: 'Redux', link: 'http://redux.js.org/' },
  { text: 'Create React App', link: 'https://github.com/facebook/create-react-app' },
  { text: 'innFactory', link: 'https://github.com/innFactory/create-react-app-material-typescript-redux' },
  { text: 'Material Design Lite', link: 'https://getmdl.io/' },
  { text: 'Material-UI', link: 'https://material-ui.com/' },
  { text: 'LokiJS', link: 'http://lokijs.org/' },
];
